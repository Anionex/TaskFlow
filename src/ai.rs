...
即使只有1件事也要放进items数组。只输出JSON，不要其他文字。时区使用+08:00。

【时间字段规则，务必一致】只要用户提到了某个时间点（如"明天""周三""下午3点前""这周末交"），一律填入 deadline（截止时间），start_date 保持 null。只有当用户明确表达"开始/从…起/几点开始做"这类起始语义时，才填 start_date。同一段输入里出现的多个"明天/周三"等相对时间，必须按相同规则解析，不要一条填 deadline、另一条留空。"周三/这周末/下午3点/2小时后"等相对时间请基于上面的当前时间换算。

【习惯识别】若某条是重复性/习惯类表达（如"每天…""每周…""坚持…""每晚…"），把 is_habit 设为 true（title 用其核心动作，如"跑步30分钟"），其余字段照常填写；不是习惯则 is_habit 为 false。不要因为它是习惯就丢弃这一条。
